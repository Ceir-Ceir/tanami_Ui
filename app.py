...
def fetch_data() -> Tuple[pd.DataFrame, pd.DataFrame]:
    """Fetch lead rollups and events from Supabase into DataFrames."""
    ...
    if not leads_df.empty and "last_seen" in leads_df:
        leads_df["last_seen"] = pd.to_datetime(leads_df["last_seen"], errors="coerce")
    if not leads_df.empty and "first_seen" in leads_df:
        leads_df["first_seen"] = pd.to_datetime(leads_df["first_seen"], errors="coerce")
...
def render_converted_lead_funnel(events_df: pd.DataFrame) -> None:
    ...
    has_lead_key = "_lead_key" in events_df.columns
    ...
    if has_lead_key:
        grouped = counts.groupby("event_type")["_lead_key"].nunique().reset_index()
        grouped = grouped.rename(columns={"_lead_key": "lead_count"})
    else:
        grouped = counts.groupby("event_type").size().reset_index(name="lead_count")

    if "created_at" in counts.columns:
        median_times = (
            counts.groupby("event_type")["created_at"].median().reset_index()
        )
        grouped = grouped.merge(median_times, on="event_type", how="left")

    grouped = grouped.sort_values("lead_count", ascending=False).head(6)
    ...
    fig = px.funnel(
        ...
        x="lead_count",
        y="event_type",
        title="Converted Lead Funnel (Top Events)",
        color_discrete_sequence=[PRIMARY_COLOR],
    )
    fig.update_layout(xaxis_title="Converted Leads", yaxis_title="Event Type")
    st.plotly_chart(fig, use_container_width=True)
...
def main() -> None:
    ...
    with tab_leads:
        st.subheader("High-Value Lead List")
        render_lead_list(leads_df, events_df)
    ...
    with tab_trends:
        ...
        with col4:
            render_converted_event_trends(converted_events)
